...
$ic->set_halt_cb(sub {
        ...
        #my $v = $g->vertex(join(',', @$target));
        my @vertices = grep { my ($x, $y) = split /,/; $map{$y}{$x} eq '.' } map { $_->[1] } $g->edges_from(join(',', @$target));
        my $minutes = 0;
        while (@vertices) {
            ...
            @vertices = grep { my ($x, $y) = split /,/; $map{$y}{$x} eq '.' } map { $_->[1] } map { $g->edges_from($_) } @vertices;
            $minutes++;
        }
